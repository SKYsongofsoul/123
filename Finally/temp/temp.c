...
{   
	...
	LPC_GPIO1->DIR&=~(1<<8);	// 设置P2.5引脚为输入引脚 
...
{
	LPC_GPIO1->DIR|=(1<<8);		   // P2.5为输出，用作DS18B20数据引脚
	LPC_GPIO1->DATA|=(1<<8);	   // P2.5置高
...
uint8_t DS18B20_Read_Bit(void) 			 
{
	...
	LPC_GPIO1->DIR|=(1<<8);	        //P2.5输出
    DQ_LOW;         //拉低DQ
	delay_ds(2);        //延时
    DQ_HIGH;                    //拉高DQ
	LPC_GPIO1->DIR&=~(1<<8);        //P2.5输入
	delay_ds(12);           //延时
	if(DQ_UP)   data=1;
        else data=0;	 
    delay_ds(50);           //延时
