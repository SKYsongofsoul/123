...
#define MAINCLKSEL_Val        0x00000001              // Reset: 0x000
...
/*----------------------------------------------------------------------------
  Define clocks
 *----------------------------------------------------------------------------*/
#define __XTAL            (10000000UL)    /* Oscillator frequency             */
...
void CLKOUT_EN(uint8_t CLKOUT_DIV)
{
	...
	LPC_IOCON->PIO0_1=0XD1;						   // °ÑP0.1½ÅÉèÖÃÎªCLKOUTÒý½Å
	LPC_SYSCON->SYSAHBCLKCTRL &= ~(1<<16);          // ½ûÄÜIOCONÊ±ÖÓ
	LPC_SYSCON->CLKOUTDIV   = CLKOUT_DIV;			//CLKOUTÊ±ÖÓÖµÎª50/CLKOUT_DIV
	LPC_SYSCON->CLKOUTCLKSEL= 0X00000003;	  		//CLKOUTÊ±ÖÓÔ´Ñ¡ÔñÎªÖ÷Ê±ÖÓ
	LPC_SYSCON->CLKOUTUEN   =0X01;
	LPC_SYSCON->CLKOUTUEN   =0X00;
	LPC_SYSCON->CLKOUTUEN   =0X01;
	while (!(LPC_SYSCON->CLKOUTUEN & 0x01));        //È·¶¨Ê±ÖÓÔ´¸üÐÂºóÏòÏÂÖ´ÐÐ
}
